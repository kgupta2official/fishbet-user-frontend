...
const BonusCard = ({ bonus, flippedCard, handleFlip }) => {
  return (
    ...
    >
      {/* Front Side */}
      <div
        className={`absolute w-full h-full bg-[rgb(var(--lb-purple-600))] rounded-lg overflow-hidden backface-hidden transform transition-transform duration-500 ${style.cardFront}`}
      >
        <Image
          src={bonus.imageUrl}
      ...
      {/* Back Side */}
      <div
        className={`absolute w-full h-full bg-[rgb(var(--lb-purple-1300))] rounded-lg transform transition-transform duration-500 ${style.cardBack}`}
      >
        <div className="flex justify-between items-center p-3 bg-[rgb(var(--lb-purple-1200))] rounded-t-lg">
        ...
        <div
          className={`p-4 text-white flex flex-col h-[calc(100%-2rem)] overflow-y-auto ${style.terms} scrollable-Content`}
        >
          <p
            dangerouslySetInnerHTML={{
